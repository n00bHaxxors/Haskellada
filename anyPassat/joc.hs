...
--dades per inicialitzar el tauler
...
-- Donat un moviment i una partida, retorna la partida resultant d'aplicar el moviment a la partida inicial
...
-- Donada una llista de linies d'un arxiu de text de partida, i una partida on aplicar-les, retorna la partida final amb els seus corresponents errors i sortida de text
...
checkJugada s p = result
    where 
      ...
      raoDeFi
        ...
        | otherwise = "Fi del fitxer - Taules"


--Donades les línies d'un fitxer de partida, i una partida inicial, retorna el resultat de la partida, i si la partida és vàlida o no
checkJugades :: [String] -> Partida -> ([String],(Bool, Partida)) 
...
--Donada una string que conté una posicio, retorna la dada "Posició" equivalent
readPos :: String -> Posicio
readPos s = Posicio (head s) (digitToInt ( head ( tail s )))

--Donada una string amb la representació d'un moviment del joc, retorna la dada "Moviment" equivalent
...
readMoviment s b = result
      ...
      peca = Peca (head (take 1 s)) b posIni
...
--Donada una string que representa una jugada retorna la dada "Jugada" representada
...
--Donada una partida i un moviment que hi volem aplicar, ens diu si és legal i també retorna la corresponent sortida per pantalla
...
checkMoviment (Partida (Tauler t) actu) (Moviment (Peca i b pos) pi pf c e m) = (pIni && pFi && captura && escac && mat && potFerMov && destapar,error )
  ...
  error = 
    ...
    else if not escac then ["La notació d'escac és incorrecta"]
    else if not mat then ["La notació d'escac i mat és incorrecta"]
...
--Donada una posicio del tauler, i una direcció on moure-la, retorna la posició resultant
...
--Donada una posicio inicial i una llista de direccions, retorna la posicio resultant
moure :: Posicio -> [Direccio] -> Posicio 
...
--Retorna totes les posicions que comparteixin columna amb la posició entrada
columnPositions :: Posicio -> [Posicio] 
columnPositions p = map (Posicio (columna p))  [1..8]

--Retorna totes les posicions que comparteixin fila amb la posició entrada
rowPositions :: Posicio -> [Posicio] 
rowPositions p = pam (map Posicio ['a'..'h']) (fila p)

--Retorna totes les posicions que comparteixin diagonal amb la posició entrada
diagonalPositions :: Posicio -> [Posicio] 
...
--Donada una llista de posicions, retorna només les posicions legals de la llista
